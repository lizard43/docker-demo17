...
var MC = MC || (function() {
    ...
    function onConnect() {
        ...
        console.log("++ Connected to MQTT Brocker websocket");

        websocketclient.subscribe("command/missile", 0,  engine.onMissile);
        websocketclient.subscribe("command/rocket", 0,  engine.onRocket);
    }
    ...
    var engine = (function() {
        ...
        function onMissile(message) {
            console.log("Recvd Missile");
            var missileMessage = JSON.parse(message);
            var missile = new Missile(missileMessage.origin, missileMessage.target,  missileMessage.speed);
        ...
        function _gameLoop() {
            ...
            //     websocketclient.publish("barf/foo", JSON.stringify(missile),  0, false);
            // }
...
//            debugInfo();
        ...
        function loadLevel(level) {
            ...
            _entities.turret[0] = new Turret(_width/2, _height/2);
            _entities.targets.push(_entities.turret[0]);

            _entities.turret[1] = new Turret(_width, _height);
            _entities.targets.push(_entities.turret[1]);
